...
func (c *ServerConn) Initialize() error {
	...
	if !ok {
		c.tcURL, ok = connectObject.GetString("tcurl")
		if !ok {
			// Some cameras don't send tcUrl - construct from app
			c.tcURL = "rtmp://localhost/" + c.app
		}
	}
...
func (c *ServerConn) Accept() error {
	...
	for {
		...
		switch cmd.Name {
		case "releaseStream", "FCPublish", "FCUnpublish", "_checkbw", "deleteStream":
			// Respond with _result to commands cameras may send before createStream.
			// Some camera firmware waits for a response before proceeding.
			err = c.mrw.Write(&message.CommandAMF0{
		...
		case "publish":
			...
			// Send StreamBegin before onStatus - some cameras wait for this
			// before they start sending media data. Wowza sends this.
			err = c.mrw.Write(&message.UserControlStreamBegin{
				StreamID: 1,
			})
			if err != nil {
