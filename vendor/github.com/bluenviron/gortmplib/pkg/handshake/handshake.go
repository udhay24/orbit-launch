...
func doServerPlain(rw io.ReadWriter, strict bool) error {
	...
	// Bundle S0+S1+S2 into a single write (3073 bytes) to minimize
	// round trips. Wowza sends these as one packet which is critical
	// for cameras with tight internal timeouts on slow networks.
	buf := make([]byte, 0, 1+c1s1Size+c2s2Size)
	buf = append(buf, s0.Version)

	s1Buf := make([]byte, c1s1Size)
	s1Buf[0] = byte(s1.Time >> 24)
	s1Buf[1] = byte(s1.Time >> 16)
	s1Buf[2] = byte(s1.Time >> 8)
	s1Buf[3] = byte(s1.Time)
	s1Buf[4] = byte(s1.Version >> 24)
	s1Buf[5] = byte(s1.Version >> 16)
	s1Buf[6] = byte(s1.Version >> 8)
	s1Buf[7] = byte(s1.Version)
	copy(s1Buf[8:], s1.Data)
	buf = append(buf, s1Buf...)

	s2Buf := make([]byte, c2s2Size)
	s2Buf[0] = byte(s2.Time >> 24)
	s2Buf[1] = byte(s2.Time >> 16)
	s2Buf[2] = byte(s2.Time >> 8)
	s2Buf[3] = byte(s2.Time)
	s2Buf[4] = byte(s2.Time2 >> 24)
	s2Buf[5] = byte(s2.Time2 >> 16)
	s2Buf[6] = byte(s2.Time2 >> 8)
	s2Buf[7] = byte(s2.Time2)
	copy(s2Buf[8:], s2.Data)
	buf = append(buf, s2Buf...)
	...
	// Skip C2 data validation - cameras with poor firmware may send
	// mismatched data. Wowza and YouTube ignore C2 validation.
	_ = strict
